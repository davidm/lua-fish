...
require "luafish.staticmodule" {
  ...
  -- note: these conditionals are being evaluated at compile time,
  -- though checked at run-time.
  assert(ISSQUARE(m))
  assert(ISSQUARE(m * 2))
  assert(not ISSQUARE(2 * m)) -- compile error (op type)
  -- assert(not ISSQUARE(m + m)) -- compile error (op not defined)
  -- assert(not ISSQUARE(m .. '')) -- compile error (op not defined)
